...
def home():
    return jsonify({
        "message": "Multi-service application with Redis and PostgreSQL",
...
def get_cache(key):
    try:
        ...
        return jsonify({"key": key, "value": value, "cached": value is not None})
...
def set_cache(key, value):
    try:
        ...
        return jsonify({"message": f"Cached {key} = {value}", "expiry": "5 minutes"})
    except Exception as error:
        return jsonify({"error": str(error)}), 500
...
def health_check():
    ...
    try:
        ...
        health_status["redis"] = f"unhealthy: {error}"
    ...
    try:
        ...
        health_status["postgres"] = f"unhealthy: {error}"

    status_code = 200 if health_status["redis"] == "healthy" and health_status["postgres"] == "healthy" else 503
